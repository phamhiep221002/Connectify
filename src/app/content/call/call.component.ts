  ...
  export class CallComponent implements OnInit, OnDestroy {
    ...
    constructor(private accountService: AccountService, private callService: CallService, private route: ActivatedRoute) {
      ...
      this.callService.incomingCall$.subscribe(incoming => {
        ...
        if (incoming) {
          // Đặt hẹn giờ để tự động từ chối cuộc gọi sau 30 giây
          this.autoDenyTimer = setTimeout(() => {
            this.denyIncomingCall();
            window.close(); // Đóng cửa sổ
          }, 20000);
        } else {
          // Hủy bỏ hẹn giờ nếu cuộc gọi được chấp nhận hoặc từ chối
          if (this.autoDenyTimer) {
            clearTimeout(this.autoDenyTimer);
          }
        }
      });
      // this.autoCallUserTimer = setTimeout(() => {
      //   this.callService.missCall(this.otherPeerId);
      //   window.close(); // Đóng cửa sổ nếu hàm callUser không được gọi trong 30s
      // }, 30000);
    ...
    handleIncomingCalls() {
      this.myPeer.on('call', (call: any) => {
        ...
        call.on('stream', (userVideoStream: MediaStream) => {
          if (this.isScreenSharing) {
            this.otherScreenVideo.nativeElement.srcObject = this.getScreenStream;  // Chỉ định màn hình chia sẻ của B hiển thị ở otherScreenVideo
